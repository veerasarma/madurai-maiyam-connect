  ...
  return (
    <a
      ...
      className={`whitespace-nowrap ${textColor} hover:text-primary transition relative group font-medium ${
        isTamil
          ? `font-tamil-ui nav-tamil-link ${compact ? "text-xs 2xl:text-[13px]" : "text-[13px] 2xl:text-sm"}`
          : "text-[13px] 2xl:text-sm inline-flex items-center min-h-8"
      }`}
    >
...
export function Navbar() {
  ...
  const rightMinWidth = isTamil
    ? "xl:min-w-[15.5rem] 2xl:min-w-[22rem]"
    : "xl:min-w-[14rem] 2xl:min-w-[18rem]";

  const trackBtnClass = `items-center justify-center whitespace-nowrap px-2.5 lg:px-3 py-1.5 rounded-full text-[11px] lg:text-xs font-medium border transition shrink-0 ${
    scrolled ? "btn-ghost-light" : "border-white/35 text-white hover:bg-white/10"
  }`;

  const submitBtnClass =
    "items-center justify-center whitespace-nowrap px-2.5 lg:px-3.5 py-1.5 rounded-full text-[11px] lg:text-xs font-semibold shrink-0";

  const headerCtaVisibility = isTamil ? "hidden xl:inline-flex" : "hidden md:inline-flex";
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6">
        <div className="relative flex items-center justify-between gap-2 h-14 lg:h-16 min-w-0">
          {/* Brand */}
          <a
            href="#"
            className="relative z-20 flex items-center gap-2 shrink-0 min-w-0 max-w-[calc(100%-5.5rem)] sm:max-w-[9.5rem]"
          >
            <img
            ...
            />
            <div className="leading-tight hidden sm:block min-w-0 overflow-hidden">
              ...
              <div
                className={`${isTamil ? "font-tamil-ui" : ""} text-[10px] leading-none mt-0.5 truncate ${
          ...
          {/* Right: language + CTAs + mobile menu */}
          <div
            className={`relative z-20 flex items-center justify-end gap-1.5 sm:gap-2 shrink-0 min-w-0 ${rightMinWidth}`}
          >
            <LanguageSwitcher scrolled={scrolled} compact={isTamil} isTamil={isTamil} />

            {/* Tamil: CTAs only in drawer below xl (same row as hamburger). English: CTAs from md up. */}
            <a
              href="#track"
              className={`${headerCtaVisibility} ${trackBtnClass} ${
                isTamil ? "font-tamil-ui nav-tamil-link nav-tamil-header-cta" : ""
              }`}
            >
              {isTamil ? (
                <>
            ...
            <a
              href="#submit"
              className={`${headerCtaVisibility} btn-glow-red ${submitBtnClass} ${
                isTamil ? "font-tamil-ui nav-tamil-link nav-tamil-header-cta" : ""
              }`}
            >
              {isTamil ? (
            ...
            <button
              ...
              aria-expanded={open}
              onClick={() => setOpen((o) => !o)}
              className="xl:hidden w-9 h-9 grid place-items-center rounded-lg glass shrink-0 ml-0.5"
            >
        ...
        <div className="xl:hidden glass border-t border-white/10 mx-3 mb-2 rounded-2xl p-4 space-y-1">
          {nav.links.map((l) => (
            <a
              ...
              className={`block ${
                isTamil ? "font-tamil-ui text-[15px] leading-snug" : ""
              } text-foreground/85 font-medium py-2.5 px-2 rounded-lg hover:bg-foreground/5`}
            >
              {l.label}
            </a>
          ))}
          <div className="flex flex-col gap-2 pt-3 mt-2 border-t border-white/10">
            <a
              href="#track"
              onClick={() => setOpen(false)}
              className={`btn-ghost-light block text-center ${
                isTamil ? "font-tamil-ui text-sm leading-snug" : ""
            ...
            <a
              href="#submit"
              onClick={() => setOpen(false)}
              className={`btn-glow-red block text-center ${
                isTamil ? "font-tamil-ui text-sm leading-snug" : ""
